...
//// START implementation
	...
	//fr_req_info.s10_mme_fteid.ip.ipv4present = true;
	fr_req_info.s10_mme_fteid.ip.ipv4.s_addr = msgData.senderFTeidForControlPlane.ipV4Address.ipValue;
	fr_req_info.s10_mme_fteid.header.iface_type = msgData.senderFTeidForControlPlane.interfaceType;
    fr_req_info.s10_mme_fteid.header.teid_gre = msgData.senderFTeidForControlPlane.teidGreKey;

    fr_req_info.srcToTargetTranspContainer.count = msgData.eUtranTransparentContainer.containerType;


    fr_req_info.header.destInstAddr = htonl(mmeAppInstanceNum_c);
    fr_req_info.header.srcInstAddr = htonl(s10AppInstanceNum_c);
