...
{
	log_msg(LOG_INFO, "GTP recv msg handler.");

	MsgBuffer* msgBuf_p = (MsgBuffer*)(message);

	uint32_t target_ip = MsgBuffer_readUint32(msgBuf_p, target_ip);
	...
	case GTP_CREATE_SESSION_RSP:
		s11_CS_resp_handler(msgBuf_p, &msgHeader, target_ip);
		break;

	case GTP_MODIFY_BEARER_RSP:
		s11_MB_resp_handler(msgBuf_p, &msgHeader, target_ip);
		break;

	case GTP_DELETE_SESSION_RSP:
		s11_DS_resp_handler(msgBuf_p, &msgHeader, target_ip);
		break;
	
	case GTP_RABR_RSP:
		s11_RB_resp_handler(msgBuf_p, &msgHeader, target_ip);
		break;
	
	case GTP_DOWNLINK_DATA_NOTIFICATION:
		s11_DDN_handler(msgBuf_p, &msgHeader, target_ip);
		break;
	
	case GTP_CREATE_BEARER_REQ:
        	s11_CB_req_handler(msgBuf_p, &msgHeader, target_ip ,src_port);
        	break;
				
    	case GTP_DELETE_BEARER_REQ:
        	s11_DB_req_handler(msgBuf_p, &msgHeader, target_ip ,src_port);
        	break;

    	case GTP_ECHO_REQ:
    		s11_ECHO_req_resp_handler(msgBuf_p, &msgHeader, target_ip,src_port);
    		break;
#ifdef S10_FEATURE
	case GTP_FORWARD_RELOCATION_REQ:
		s10_FR_req_handler(msgBuf_p, &msgHeader, target_ip);
	     break;
		 
	case GTP_FORWARD_RELOCATION_RES:
		 s10_FR_resp_handler(msgBuf_p, &msgHeader, target_ip);
		 break;
	
	case GTP_FORWARD_RELOCATION_COMPLETE_NOTIFICATION:
		 s10_FR_relocation_complete_notification(msgBuf_p, &msgHeader, target_ip);
		 break;
	
	case GTP_IDENTIFICATION_RES:
		s10_ID_resp_handler(msgBuf_p, &msgHeader, target_ip);
		break;
		
	case GTP_IDENTIFICATION_REQ:
		s10_ID_request_handler(msgBuf_p, &msgHeader, target_ip);
		break;
	case GTP_FORWARD_ACCESS_CONTEXT_NOTIFICATION:
		S10_fwd_acc_ctxt_noti_handler(msgBuf_p, &msgHeader, target_ip);
		break;
	case GTP_FORWARD_RELOCATION_CMP_ACK:
		s10_FWD_relocation_cmp_ack_handler(msgBuf_p, &msgHeader, target_ip);
		break;
	case GTP_FORWARD_ACCESS_CONTEXT_ACKNOWLEDGE:
		s10_Fwd_acc_ctxt_ack_handler(msgBuf_p, &msgHeader, target_ip);
		break;		
#endif
	}
