...
{
	/*s1ap information*/
	g_gtp_cfg.local_egtp_ip = get_ip_scalar("s11.egtp_local_addr");
	if(-1 == g_gtp_cfg.local_egtp_ip) return -1;
	g_gtp_cfg.egtp_def_port = get_int_scalar("s11.egtp_default_port");
	if(-1 == g_gtp_cfg.egtp_def_port) return -1;
	g_gtp_cfg.target_mme_ip = get_ip_scalar("s10.target_mme_ip");
	if(-1 == g_gtp_cfg.target_mme_ip) return -1;
	...
	struct local_config config_addr[] = 
	{
		{ 
		  .name = "s11.sgw_addr",
		  .addr = &g_gtp_cfg.sgw_ip,
		},
		{ 
		  .name = "s11.pgw_addr",
		  .addr = &g_gtp_cfg.pgw_ip,
		}
	};
